...
- [x] **5.1 不要コードの削除**
    ...
    - [x] グローバルスコープに残った旧関数（`clampDest`, `resolveMovement`, `npcAttack` など）を完全に削除する。
        
- [x] **5.2 最終テスト**
    
    - [x] コンソールログにエラーや警告が出ていないか確認する。
        
    - [x] ミッションクリアまでプレイし、増援の出現、スコア加算、ハイスコア保存など、ゲームループ全体が破綻なく動作することを検証する。
